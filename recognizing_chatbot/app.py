...
# Admin setup
admin = Admin(app, name='sketch_recognizing', template_mode='bootstrap3')
...
def transform():
    ...
    strokes = image_data['strokes']
    box = image_data['box']

    filepath = "./images/" + str(uuid4()) + ".png"
    img = transform_img(strokes, box)
    img.save(filepath)
...
# app.add_url_rule('/', endpoint='static', view_func=app.send_static_file)
...
def submit_screenshot():
    ...
    if user_id and image_data:
        ...
        filename = f'{user.username}_recognizing.{ext}'
